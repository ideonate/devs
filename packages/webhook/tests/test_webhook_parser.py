    ...
    def test_extract_task_description(self):
        ...
        payload = {
            ...
            "issue": {
                ...
                "number": 123,
                "title": "Fix login bug",
                "body": "@testuser there's a bug in the login system",
                "state": "open",
                "user": {
                    "login": "reporter",
                    "id": 456,
        ...
        event = WebhookParser.parse_webhook(headers, payload_bytes)
        task_description = WebhookParser.extract_task_description(event, "testuser")
        
        assert "Fix login bug" in task_description
        assert "@testuser" in task_description
        assert "test/repo" in task_description
        assert "You have been mentioned" in task_description